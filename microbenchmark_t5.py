...
from megatron.schedules import forward_step, backward_step
...
def enc_output_fw_hook():
    ...
    stat_recorder.add("memory_after_enc_forward", memory_after_enc_fw)
    stat_recorder.add("peak_memory_after_enc_forward", peak_memory_after_enc_fw)
...
def model_provider(pre_process=True, post_process=True,
    ...
    model = T5Model(num_tokentypes=0,
                    ...
                    enc_output_grad_hook=enc_output_grad_hook)
    return model
...
def get_batch(data_iterator):
    ...
    microbatch_size, enc_sequence_length, dec_sequence_length = next(data_iterator)
    datatype = torch.int64
    # generate random data
    data_b = {
        'text_enc': torch.randint(0, 32000, (microbatch_size, enc_sequence_length), dtype=datatype),
        'text_dec': torch.randint(0, 32000, (microbatch_size, dec_sequence_length), dtype=datatype),
        'labels': torch.randint(0, 32000, (microbatch_size, dec_sequence_length), dtype=datatype),
        'loss_mask': torch.ones((microbatch_size, dec_sequence_length), dtype=datatype),
        'enc_mask': torch.ones((enc_sequence_length, enc_sequence_length), dtype=datatype),
        'dec_mask': torch.ones((dec_sequence_length, dec_sequence_length), dtype=datatype),
        'enc_dec_mask': torch.ones((dec_sequence_length, enc_sequence_length), dtype=datatype)
    }
...
def benchmark_forward_backward_no_pipelining(forward_step_func,
                                   data_iterator, model,
                                   optimizer,
                                   timers,
                                   forward_only,
    ...
    stat_recorder.add("memory_before_forward", memory_before_forward)
    stat_recorder.add("peak_memory_before_forward", peak_memory_before_forward)
    ...
    stat_recorder.add("memory_after_dec_forward", memory_after_forward)
    stat_recorder.add("peak_memory_after_dec_forward", peak_memory_after_forward)
    if not forward_only:
        ...
        stat_recorder.add("memory_after_backward", memory_after_backward)
        stat_recorder.add("peak_memory_after_backward", peak_memory_after_backward)

    return forward_data_store

def benchmark_train_step(forward_step_func, data_iterator,
...
def benchmark_train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    assert args.train_iters >= WARMUP_ITERATIONS, \
        "train_iters must be greater than or equal to {} for benchmarking".format(WARMUP_ITERATIONS)

    while iteration < args.train_iters:
        update_num_microbatches(args.consumed_train_samples)
        args.curr_iteration = iteration
        loss_dict, skipped_iter, grad_norm, num_zeros_in_grad = \
            benchmark_train_step(forward_step_func,
                       benchmark_shape_iterator,
                       model,
                       optimizer,
                       opt_param_scheduler)
...
def get_optimizer_state_size(optimizer):
    ...
    state_size = 0
    for per_tensor_states in optimizer.state_dict()['state'].values():
...
def generate_report(n_iters):
    ...
    def _get_stats_and_print(attr_name):
        ...
        if val:
            _rprint("    {attr_name}: {:.2f} MB".format(val), " ")
    def _get_time_and_print(attr_name):
        val = timers(attr_name).elapsed(reset=False)
        if val:
            _rprint("    {attr_name}: {val:.2f} ms".format(val / n_iters * 1000), " ")
    def _get_time_and_print_difference(attr_name1, attr_name2, new_name):
        val = timers(attr_name1).elapsed(reset=False) - timers(attr_name2).elapsed(reset=False)
        if val:
            _rprint("    {}: {val:.2f} ms".format(new_name, val / n_iters * 1000), " ")
    ...
    _get_time_and_print_difference("backward_total", "backward_decoder", "backward_encoder")
...
def microbenchmark(
    ...
    if hasattr(model, "embedding"):
        model_embedding_param_size = _get_param_size(model.embedding.parameters())
        stat_recorder.add("model_embedding_param_size", model_embedding_param_size)
    ...
    args.train_iters = profile_iters

    iteration = 0
    iteration = benchmark_train(forward_step_func,
                        model, optimizer, opt_param_scheduler,
                        benchmark_shape_provider())
...
if __name__ == "__main__":
    microbenchmark(train_shape_provider, model_provider, ModelType.encoder_and_decoder,
             forward_step_func, args_defaults={'tokenizer_type': 'BertWordPieceLowerCase'})
