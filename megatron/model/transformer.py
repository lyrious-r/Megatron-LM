...
class ParallelMLP_DRC(MegatronModule):
    """MLP.
...
class ParallelAttention(MegatronModule):
    ...
    def __init__(self, init_method,
        ...
        # disable flash attention for decoder layers
        self.use_flash_attn = args.use_flash_attn and self.attn_mask_type == AttnMaskType.causal
    ...
    def forward(self, hidden_states, attention_mask,
                encoder_output=None, inference_params=None, select_rc=False):
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def forward(self, hidden_states, attention_mask,
                ...
                inference_params=None, select_rc = False):
        # hidden_states: [s, b, h]
...
class ParallelTransformer(MegatronModule):
    ...
    def _dynamic_checkpointed_forward(self, hidden_states, attention_mask,
                              encoder_output, enc_dec_attn_mask, is_first_microbatch,recompute_policy):
        """Forward method with activation checkpointing."""
        def custom(start, end, is_transformer_engine=False):
            def custom_forward(*args, **kwargs):
                for index in range(start, end):
        ...
        # print('num_layers:',self.num_layers)
            ...
            for i in recompute_policy:
                if self.transformer_impl == 'transformer_engine':
                    print('transformer_engine')
                    hidden_states = transformer_engine.pytorch.distributed.checkpoint(
                        custom(i[0], i[1], is_transformer_engine=True),
                        self.distribute_saved_activations,
                        tensor_parallel.get_cuda_rng_tracker,
                        mpu.get_tensor_model_parallel_group(),
                        hidden_states, attention_mask, encoder_output, enc_dec_attn_mask)
                    ...
                    if i[2] == 'None':
                        hidden_states = custom(i[0], i[1])(hidden_states, attention_mask,
                        ...
                        hidden_states = custom(i[0], i[1])(hidden_states, attention_mask,
                                encoder_output, enc_dec_attn_mask, select_rc=True)
                    else:
                        raise ValueError("Invalid recompute policy.")
        ...
        with rng_context:
            ...
            with transformer_engine.pytorch.fp8_autocast(
                ...
                if mpu.get_recomputation_level() == 'full':
                    if args.drc:
                        # print('Use Dynamic Recomputation')
                        if recompute_policy is None:
                            recompute_policy = [(0,1,'Full'),(1,2,'Full'),(2,3,'Full'),(3,4,'Full'),(4,5,'Full'),(5,6,'Full'),(6,7,'Full'),(7,8,'Full')]
                        hidden_states = self._dynamic_checkpointed_forward(hidden_states,
                                                attention_mask,
                                                encoder_output,
                                                enc_dec_attn_mask,
                                                is_first_microbatch,
