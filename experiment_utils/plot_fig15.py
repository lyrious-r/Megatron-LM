...
FIG15_LEFT_GBS = 65536
for model in MODELS:
    model_df = df[(df["model"] == model) & (df["global_batch_size"] == FIG15_LEFT_GBS)].copy()
    if model == "GPT":
    ...
    plt.savefig(os.path.join(args.out_dir, f"fig15_{subfig}_left.pdf"), bbox_inches='tight')


# plot right figure (fix seqlen, vary global batch size)
FIG15_RIGHT_SEQLEN = 2048
for model in MODELS:
    model_df = df[(df["model"] == model) & (df["seqlen"] == FIG15_RIGHT_SEQLEN)].copy()
    if model == "GPT":
        model_df = model_df[model_df["Type"] == "Encoder"]
        for row in model_df.itertuples():
            model_df.at[row.Index, "Type"] = row.framework
    ...
    subfig = "a" if model == "GPT" else "b"
    plt.savefig(os.path.join(args.out_dir, f"fig15_{subfig}_right.pdf"), bbox_inches='tight')